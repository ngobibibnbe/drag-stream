...
#sys.argv =["LAMP-conference_code/train_neural_net_LAMP.py", 200, "dataset/test_tmp.mat", "./logs"]
if len(sys.argv) < 4:
...
lookahead_seconds = 0
subsequence_stride = 1# 256
lookbehind = sample_rate * lookbehind_seconds
num_outputs = 32*int(int(sys.argv[1])/8)#400#256
...
if training:
  ...
  # Construct generators for the model inpulot
  print("input modele:", input_width, n_input_series, subsequences_per_input)
  print("stride:",subsequence_stride, "mp window:",matrix_profile_window, "internal_stride:",channel_stride  )
  train_gen =  MPTimeseriesGenerator(ts, mp, num_input_timeseries=n_input_series, stride=subsequence_stride, internal_stride=channel_stride, num_outputs=num_outputs, lookahead=forward_sequences, lookbehind=lookbehind, length=input_width, mp_window=matrix_profile_window, important_upper_threshold=high_thresh, important_lower_threshold=low_thresh, important_weight=high_weight, batch_size=batch_size, shuffle=shuffle, merge_points=merge_points)
...
if predicting:
  ...
  directory = os.path.dirname(('LAMP-conference_code/predict/predicted_matrix_profile_'+sys.argv[4]+'.txt'))
  if not os.path.exists(directory):
      os.makedirs(directory)

  np.savetxt(R'LAMP-conference_code/predict/predicted_matrix_profile_'+sys.argv[4]+'.txt', predictions, fmt='%.10f')
...
# if len(sys.argv) < 4:
#     print(
#         'Usage: LAMP.py <mp_window_size> <root_data_dir> <root_logging_dir> <pretrained_weights_file (optional when training)> <initial_epoch (optional, default=0)>'
#     )
#     exit(1)
...
# if training:
#     dataset_name = os.path.split(root_data_path)[-1]
#     logging_filename = 'dataset={}_width={}_optimizer={}_initlr={}_batchsz={}_stride={}_shuffle={}_lookbehind={}_lookahead={}_channelstride={}_outputs={}_stateful={}_weight={}_lowthresh={}_highthresh={}_started={}'.format(
#         dataset_name, input_width, optimizer_id, initial_lr, batch_size,
#         subsequence_stride, shuffle, lookbehind, lookahead, channel_stride,
#         num_outputs, stateful_rnn, high_weight, low_thresh, high_thresh,
#         str(datetime.datetime.now()).replace(' ', '-'))
#     model_path = os.path.join(root_logging_path, 'models', logging_filename)
#     tensorboard_path = os.path.join(root_logging_path, 'tensorboard_logs',
#                                     logging_filename)
#     logging_path = os.path.join(root_logging_path, 'csv_logs',
#                                 logging_filename)
#     logging_dir = os.path.join(root_logging_path, 'csv_logs')
#     # Make the directories if they don't already exists
#     if not os.path.exists(logging_dir):
...
#         #Change model as needed
#         model = build_resnet(
#             [input_width, n_input_series, subsequences_per_input], 96,
#             num_outputs)
#     model.summary()
#     # Compile model
...
#     train_gen = MPTimeseriesGenerator(ts,
#                                       mp,
#                                       num_input_timeseries=n_input_series,
#                                       internal_stride=channel_stride,
#                                       num_outputs=num_outputs,
#                                       lookahead=forward_sequences,
#                                       lookbehind=lookbehind,
#                                       length=input_width,
#                                       mp_window=matrix_profile_window,
#                                       stride=subsequence_stride,
#                                       important_upper_threshold=high_thresh,
#                                       important_lower_threshold=low_thresh,
#                                       important_weight=high_weight,
#                                       batch_size=batch_size,
#                                       shuffle=shuffle,
...
#                                                  period=1)
#     # Function for logging progress to tensorboard
#     tensorb = TensorBoard(log_dir=tensorboard_path)
#     # Function for logging progress to CSV
#     logging = keras.callbacks.CSVLogger(logging_path)
#     # Train model
...
#                                callbacks=[checkpoint, tensorb, logging])
...
#     mp = np.zeros((len(ts) - matrix_profile_window + 1, 1))
#     # Construct generator for test data
#     test_gen = MPTimeseriesGenerator(ts,
#                                      mp,
#                                      num_input_timeseries=n_input_series,
#                                      internal_stride=channel_stride,
#                                      num_outputs=num_outputs,
...
#                                           workers=6)
#     predictions = predictions.flatten()
#     np.savetxt(R'LAMP-conference_code/predicted_matrix_profile.txt', predictions, fmt='%.10f')
