...
class MPTimeseriesGenerator(Sequence):
    ...
    def __init__(self,
                 ...
                 autoencoder=False,
                 merge_points=None):
        print(lookahead,lookbehind,internal_stride)
        if (lookahead + lookbehind) % internal_stride != 0:
            raise ValueError(
                "Internal stride must evenly divide number of input subsequences"
            )
        ...
        if self.lookahead < self.num_outputs:
            raise ValueError(
                'Lookahead must be greater than or equal to the number of timesteps required by the output length'
            )
    ...
    def __len__(self):
        ...
        return math.ceil((self.end_index - self.start_index) /
    ...
    def subsequences(self, x, m, striding, limit):
        ...
        if n is not limit:
            raise ValueError(
                'Problem encountered in subsequences, n = {}, len(x) = {}'.
                format(n, len(x)))
        s = x.itemsize
        return as_strided(x,
                          shape=(n, m),
                          strides=(s * striding, s),
                          writeable=False)
    ...
    def __getitem__(self, index):
        ...
        rows = self.indexes[index * self.batch_size:min(
            (index + 1) * self.batch_size, len(self.indexes))]
        # Allocate space
        samples, targets = self._empty_batch(len(rows), self.num_steps,
                                             self.num_outputs)
        ...
        for i in range(self.n_series):
            ...
            for j, row in enumerate(rows):
                ...
                samples[j, :, i, :] = self.subsequences(
                    self.data[row - self.lookbehind:row + self.lookahead +
                              self.sublen, i], self.sublen,
                    self.internal_stride, self.num_steps)
    ...
    def on_epoch_end(self):
        self.indexes = np.arange(self.start_index, self.end_index + 1,
                                 self.stride)
        ...
        if self.merge_points:
            ...
            for pos in self.merge_points:
                block_behind = (pos - self.start_index -
                                self.lookahead) // self.stride
                block_front = math.ceil(
                    (pos - self.start_index + 1 + self.lookbehind) /
                    self.stride)
        ...
        if self.shuffle:
            self.indexes = self.perturb(self.indexes, self.stride,
                                        self.end_index)
